...
class Server {
  ...
  def routes[F[_]: Bracket[*[_], Throwable]](endpoints: Endpoints[F])(
      implicit ev: Sync[Kleisli[F, Span[F], *]]
  ): HttpRoutes[Kleisli[F, Span[F], *]] = _routesBetterExampleOfRealUseCase(endpoints)
  ...
  def getTransactor[F[_]: Sync]: Resource[F, Transactor[F]] =
    Resource.make(Sync[F].pure(new Transactor[F] {}))(_ => Sync[F].unit)
  ...
  def app[F[_]: Sync: Bracket[*[_], Throwable]](ep: EntryPoint[F], xa: Transactor[F]): HttpApp[F] = {
    val endpoints = getEndpoints[F](xa)
    val baseApp = routes(endpoints).orNotFound
  ...
  def run[F[_]: ConcurrentEffect: Bracket[*[_], Throwable]: Timer]: F[ExitCode] = {
    for {
      entryPoint <- fs2.Stream.resource(entryPointResource)
      xa <- fs2.Stream.resource(getTransactor[F])
